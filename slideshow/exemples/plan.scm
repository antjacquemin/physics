...
;                      OUTILS 
; Défintion de la projection du point (xM yM)
;              sur la droite (AB)
...
(define masse%
  (class point%
    ...
    (define/public (chocx-objet m1 m2)
...
(define FRAME
  (new frame%
       (label "Chute d'une balle sur un plan incliné")
...
(define CANVAS
  (new canvas%
       ...
       (paint-callback (lambda (obj dc)
                         ...
                         (action dc)
                         (send (send CANVAS get-dc) draw-line 0 (/ HEIGHT 2.5) (* 0.9 WIDTH) HEIGHT)))))
  ...
  (if 
   ...
   ACTION
      (begin
        ...
        (send (send CANVAS get-dc) draw-line 0 (/ HEIGHT 2.5) (* 0.9 WIDTH) HEIGHT)
        ;si la masse est à une distance du plan d'environ son rayon, il y a choc
        (for-each (lambda (x) 
                    (let* ((p1 (projection 0 (/ HEIGHT 2.5) (* 0.9 WIDTH) HEIGHT (send x x) (send x y))) (p (make-vect (car p1) (cdr p1))))
                      (if (<= (norme (+vect p (*vect -1 (+vect (send x v) (send x r))))) (+ 1 (send x rayon)))
                          (let ((c (send x chocx-objet x (new masse% (masse 1e100) (vitesse (make-vect 0 0)) (position p)))))
                            (send x change WIDTH HEIGHT 0)
                            (send x set-v! (car c)))
                          (send x change WIDTH HEIGHT GRAVITE)))) WORLD)
        ;on affiche les masses
        (map (lambda (x) (send x play dc)) WORLD)
        (sleep/yield CLOCK)
        (action dc))
      ;sinon, on se contente de dessiner le monde
      (begin
        (send (send CANVAS get-dc) clear)
        (send (send CANVAS get-dc) draw-line 0 (/ HEIGHT 2.5) (* 0.9 WIDTH) HEIGHT)
...
(define (init-world)
  (set! WORLD '())
  (set! GRAVITE 0.1)
  (set! AFFICHE-CENTRE? #f)
  
  (set! WORLD (cons (new masse% (ray 50) (masse 1) (couleur "blue") (position (make-vect 200 100)) (vitesse (make-vect 0 0))) WORLD)))
